...
class Menu:
    ...
    def render_main_menu(self, display, font):
        ...
        title = self.render_text("Dietician", font, 100, WHITE)

        # select Select Mode in the menu
        if self.menu_state == SELECT_MENU:
            text_sm = self.render_text(
                SELECT_MENU + " " * 8 + "1", font, 75, YELLOW)
        else:
            text_sm = self.render_text(
                SELECT_MENU + " " * 8 + "1", font, 75, WHITE)
    ...
    def render_select_menu(self, display, font):
        ...
        if self.menu_state == VIT_C:
            text_vc = self.render_text(
                VIT_C + " " * 16 + "1", font, 75, YELLOW)
        else:
            text_vc = self.render_text(VIT_C + " " * 16 + "1", font, 75, WHITE)

        # select Calcium in the menu
        if self.menu_state == CALCIUM:
            text_ca = self.render_text(
                CALCIUM + " " * 13 + "2", font, 75, YELLOW)
        else:
            text_ca = self.render_text(
                CALCIUM + " " * 13 + "2", font, 75, WHITE)

        # select Fibre in the menu
        if self.menu_state == FIBRE:
            text_fibre = self.render_text(
                FIBRE + " " * 16 + "3", font, 75, YELLOW)
        else:
            text_fibre = self.render_text(
                FIBRE + " " * 16 + "3", font, 75, WHITE)

        if self.menu_state == BACK:
            text_back = self.render_text(
                BACK + " " * 16 + "4", font, 75, YELLOW)
        else:
            text_back = self.render_text(
                BACK + " " * 16 + "4", font, 75, WHITE)
    ...
    def render_ready_menu(self, display, font):
        ...
        if self.menu_state == START:
            text_start = self.render_text(
                START + " " * 16 + "1", font, 75, YELLOW)
        else:
            text_start = self.render_text(
                START + " " * 16 + "1", font, 75, WHITE)
        ...
        if self.menu_state == BACK:
            text_back = self.render_text(
                BACK + " " * 17 + "3", font, 75, YELLOW)
        else:
            text_back = self.render_text(
                BACK + " " * 17 + "3", font, 75, WHITE)
    ...
    def rednder_leaderboard(self, display, font):
        ...
        for i in range(1, len(username) + 1):
            ...
            if username[f"{i}"] != time["username"]:
                username[f"{i}"] = self.render_text(
                    username[f"{i}"], font, 35, WHITE)
                score[f"{i}"] = self.render_text(
                    score[f"{i}"], font, 35, WHITE)
            else:

                username[f"{i}"] = self.render_text(
                    username[f"{i}"], font, 45, YELLOW)
                score[f"{i}"] = self.render_text(
                    score[f"{i}"], font, 45, YELLOW)
